...
def create_file_copy(ctx, rd, prefix, library_id, book_id, ext, mtime, copy_func, extra_etag_data=''):
    ...
    bname = '{}-{}-{:x}.{}'.format(prefix, library_id, book_id, ext)
...
def book_filename(rd, book_id, mi, fmt, as_encoded_unicode=False):
    ...
    ext = (fmt or '').lower()
    fname = '{} - {}_{}.{}'.format(title[:30], au[:30], book_id, ext)
    ...
    with db.safe_read_lock:
        ...
        if what == 'thumb':
            ...
            try:
                ...
                raise HTTPNotFound('No {} format for the book {!r}'.format(what.lower(), book_id))
