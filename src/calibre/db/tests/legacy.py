...
def compare_argspecs(old, new, attr):
    ...
    if not ok:
        raise AssertionError('The argspec for {} does not match. {!r} != {!r}'.format(attr, old, new))
...
def run_funcs(self, db, ndb, funcs):
    for func in funcs:
        ...
        if callable(meth):
            ...
            self.assertEqual(res1, res2, 'The method: {}() returned different results for argument {}'.format(meth, args))
# }}}
        ...
        for meth, args in iteritems({
            ...
            for a in args:
                self.assertEqual(fmt(getattr(db, meth)(*a)), fmt(getattr(ndb, meth)(*a)),
                                 'The method: {}() returned different results for argument {}'.format(meth, a))
    ...
    def test_legacy_conversion_options(self):  # {{{
        ...
        for x in (
            ...
            self.assertEqual(
                ...
                'The method: {}() returned different results for argument {}'.format(meth, args)
            )
        db.close()
    # }}}
