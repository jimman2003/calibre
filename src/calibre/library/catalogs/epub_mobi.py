    ...
    def run(self, path_to_output, opts, db, notification=DummyReporter()):
        ...
        try:
            if float(opts.thumb_width) < float(self.THUMB_SMALLEST):
                log.warning("coercing thumb_width from '{}' to '{}'".format(opts.thumb_width, self.THUMB_SMALLEST))
                opts.thumb_width = self.THUMB_SMALLEST
            if float(opts.thumb_width) > float(self.THUMB_LARGEST):
                log.warning("coercing thumb_width from '{}' to '{}'".format(opts.thumb_width, self.THUMB_LARGEST))
                opts.thumb_width = self.THUMB_LARGEST
            opts.thumb_width = "%.2f" % float(opts.thumb_width)
        except:
            log.error("coercing thumb_width from '{}' to '{}'".format(opts.thumb_width, self.THUMB_SMALLEST))
        ...
        for key in keys:
            if key in ['catalog_title', 'author_clip', 'connected_kindle', 'creator',
                ...
                build_log.append("  {}: {}".format(key, repr(opts_dict[key])))
        if opts.verbose:
