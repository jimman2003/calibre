...
class PluginUpdaterDialog(SizePersistedDialog):
    ...
    def update_forum_label(self):
        txt = ''
        if self.forum_link:
            txt = '<a href="{}">{}</a>'.format(self.forum_link, self.forum_label_text)
    ...
    def _download_zip(self, plugin_zip_url):
        ...
        raw = get_https_resource_securely(plugin_zip_url, headers={'User-Agent':'{} {}'.format(__appname__, __version__)})
