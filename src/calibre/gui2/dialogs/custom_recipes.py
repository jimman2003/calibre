...
def options_to_recipe_source(title, oldest_article, max_articles_per_feed, feeds):
    ...
    if feeds:
        if len(feeds[0]) == 1:
            feeds = '\n'.join('{}{},'.format(indent, py3_repr(url)) for url in feeds)
        else:
            feeds = '\n'.join('{}({}, {}),'.format(indent, py3_repr(title), py3_repr(url)) for title, url in feeds)
    else:
        feeds = ''
    if feeds:
        feeds = 'feeds          = [\n%s\n    ]' % feeds
...
class BasicRecipe(QWidget):  # {{{
    ...
    def add_feed(self):
        ...
        QListWidgetItem('{} - {}'.format(title, url), self.feeds).setData(Qt.ItemDataRole.UserRole, (title, url))
...
class CustomRecipes(Dialog):
    ...
    def import_opml(self):
        ...
        if failed_recipes:
            det_msg = '\n'.join('{}\n{}\n'.format(title, tb) for title, tb in iteritems(failed_recipes))
