...
class Container(ContainerBase):  # {{{
    ...
    def rename(self, current_name, new_name):
        ...
        if self.exists(new_name) and (new_name == current_name or new_name.lower() != current_name.lower()):
            ...
            raise ValueError('Cannot rename {} to {} as {} already exists'.format(current_name, new_name, new_name))
        new_path = self.name_to_abspath(new_name)
        base = os.path.dirname(new_path)
        if os.path.isfile(base):
            raise ValueError('Cannot rename {} to {} as {} is a file'.format(current_name, new_name, base))
        if not os.path.exists(base):
            os.makedirs(base)
