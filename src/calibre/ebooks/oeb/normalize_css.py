...
def normalize_edge(name, cssvalue):
    ...
    if '-' in name:
        ...
        for edge, value in zip(EDGES, values):
            style['{}-{}-{}'.format(l, edge, r)] = value
    else:
        for edge, value in zip(EDGES, values):
            style['{}-{}'.format(name, edge)] = value
    return style
...
def normalize_filter_css(props):
    ...
    for prop in props:
        ...
        if n is not None and prop in SHORTHAND_DEFAULTS:
            dec = p.parseStyle('{}: {}'.format(prop, SHORTHAND_DEFAULTS[prop]))
...
def condense_border(style, props):
    ...
    for edge in EDGES:
        ...
        for prop in BORDER_PROPS:
            x = prop_map.get('{}-{}'.format(name, prop), None)
            if x is not None:
                vals.append(x)
        if len(vals) == 3:
            for prop in vals:
    ...
    class TestNormalization(unittest.TestCase):
        ...
        def test_border_normalization(self):
            def border_edge_dict(expected, edge='right'):
                ans = {'border-{}-{}'.format(edge, x): DEFAULTS['border-{}-{}'.format(edge, x)] for x in ('style', 'width', 'color')}
                for x, v in iteritems(expected):
                    ans['border-{}-{}'.format(edge, x)] = v
                return ans
            ...
            def border_val_dict(expected, val='color'):
                ans = {'border-{}-{}'.format(edge, val): DEFAULTS['border-{}-{}'.format(edge, val)] for edge in EDGES}
                for edge in EDGES:
                    ans['border-{}-{}'.format(edge, val)] = expected
                return ans
            ...
            for raw, expected in iteritems({
                ...
                for edge in EDGES:
                    br = 'border-%s' % edge
                    val = tuple(parseStyle('{}: {}'.format(br, raw), validate=False))[0].cssValue
            ...
            for raw, expected in iteritems({
                ...
                val = tuple(parseStyle('{}: {}'.format('border', raw), validate=False))[0].cssValue
            ...
            for name, val in iteritems({
                ...
                cval = tuple(parseStyle('border-{}: {}'.format(name, val), validate=False))[0].cssValue
        ...
        def test_edge_normalization(self):
            def edge_dict(prefix, expected):
                return {'{}-{}'.format(prefix, edge) : x for edge, x in zip(EDGES, expected)}
            for raw, expected in iteritems({
                '2px': ('2px', '2px', '2px', '2px'),
                ...
                for prefix in ('margin', 'padding'):
                    cval = tuple(parseStyle('{}: {}'.format(prefix, raw), validate=False))[0].cssValue
        ...
        def test_filter_css_normalization(self):
            ...
            bvals = {'border-{}-{}'.format(edge, x) for edge in EDGES for x in BORDER_PROPS}
            ae(bvals | {'border'}, normalize_filter_css({'border'}))
            for x in BORDER_PROPS:
                sbvals = {'border-{}-{}'.format(e, x) for e in EDGES}
                ae(sbvals | {'border-%s' % x}, normalize_filter_css({'border-%s' % x}))
            for e in EDGES:
                sbvals = {'border-{}-{}'.format(e, x) for x in BORDER_PROPS}
        ...
        def test_edge_condensation(self):
            for s, v in iteritems({
                ...
                for prefix in ('margin', 'padding'):
                    css = {'{}-{}'.format(prefix, x) : unicode_type(y)+'pt' if isinstance(y, numbers.Number) else y
                            for x, y in zip(('left', 'top', 'right', 'bottom'), s)}
                    css = '; '.join(('{}:{}'.format(k, v) for k, v in iteritems(css)))
                    ...
                    if val is not None:
                        for edge in EDGES:
                            self.assertFalse(getattr(style.getProperty('{}-{}'.format(prefix, edge)), 'value', None))

        def test_border_condensation(self):
            vals = 'red solid 5px'
            css = '; '.join('border-{}-{}: {}'.format(edge, p, v) for edge in EDGES for p, v in zip(BORDER_PROPS, vals.split()))
            style = parseStyle(css)
            condense_rule(style)
            for e, p in product(EDGES, BORDER_PROPS):
                self.assertFalse(style.getProperty('border-{}-{}'.format(e, p)))
            ...
            css = '; '.join('border-{}-{}: {}'.format(edge, p, v) for edge in ('top',) for p, v in zip(BORDER_PROPS, vals.split()))
            style = parseStyle(css)
            condense_rule(style)
            self.assertEqual(css_text(style), 'border-top: %s' % vals)
            css += ';' + '; '.join('border-{}-{}: {}'.format(edge, p, v) for edge in ('right', 'left', 'bottom') for p, v in
